...
fn main() -> std::io::Result<()> {
    ...
    ];
    {
        ...
        println!(
            ...
            e.state().get_int("balance"),
            e.state().get_int("name_len"),
            e.stats.applied,
            e.stats.snapshots
        );
    } // drop = crash
    ...
    assert_eq!(e.state().get_int("balance"), Some(120));
    assert_eq!(e.state().get_int("name_len"), Some(8));
    ...
    assert_eq!(e.state().get_int("balance"), Some(120), "duplicate INCR not double-counted");
