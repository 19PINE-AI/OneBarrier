...
input's timestamp. The fabric crosses that $\sim\!1.5$-RTT barrier to deliver the
input reliably \emph{regardless of fault tolerance}, and the 1-RTT replication
completes within it. The output-commit hold and the reliable-delivery barrier are
thus the \emph{same} barrier, so FT's marginal latency over the reliable-fabric
baseline is $\approx 0$.
...
\S\ref{sec:overview}). On recovery, a replacement
...
via the per-client high-water-mark makes the replayed effects exactly-once.
...
\Procedure{Recover}{}
  ...
  \State state-transfer any missing prefix from a survivor via the recovery cut
\EndProcedure
\end{algorithmic}
\end{algorithm}

The guarantee is in-memory $f$-of-$k$ fail-stop tolerance: acknowledged writes
survive up to $f$ correlated failures among $k$ replicas, with durability supplied by
in-fabric replication rather than stable storage.
...
recovery; it ships 19 binaries (servers, benchmarks, fault injectors, checkers) and
...
$\approx 0$ (\S\ref{sec:eval-overlap})? Do real unmodified servers recover
...
each panel) the durable-replica write (green) is hidden \emph{under} a barrier the
fabric already crosses, adding $\approx 0$; when it is serial (bottom bar) it stacks
on top (red). Left: the measured loopback reproduction, where the in-fabric write
adds $0.23\%$ and serial fsync doubles commit latency. Right: the same shape
projected to 1Pipe's RDMA operating point, where a $\sim$1.5\,\textmu s replica write
hides under the $\sim$21\,\textmu s reliable-delivery barrier ($\approx 0\%$) while
out-of-regime serial durability ($\sim$100\,\textmu s) stacks. The \emph{shape}, not
the absolute scale, is the result.}
\label{fig:overlap}
\end{figure}

Output commit and reliable delivery are the same barrier, so the marginal cost is
$0.23\%$ of delivery latency (Figure~\ref{fig:overlap}). Serial fsync instead stacks
$\sim$3\,ms and saturates the executor---it completed only $3426/6000$ ops at full
load---so stable-storage durability is out of regime at fabric speed. A
...
\label{fig:softroce}
\end{figure}
The verbs path is real and its per-operation latency sits in regime; the overlap
itself is established by the model (Figure~\ref{fig:overlap}, right), since SoftRoCE
is CPU-bound rather than RTT-bound. Closing this with a hardware RDMA/P4 testbed is
the single result that would most strengthen the paper.
...
unmodified workloads fault-tolerant: Remus~\cite{cully2008remus} checkpoints a VM
...
processors (Flink~\cite{carbone2015flink}). OneBarrier targets the unmodified fleet
...
share-nothing sharding. The microsecond operating point is set by RDMA
