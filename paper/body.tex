...
empty-channel checkpoint (point (b) of \S\ref{sec:overview}). The durable-replica
write that makes an output committable is issued as phase~1 of the fabric's
reliable-delivery 2PC (point (c) of \S\ref{sec:overview}), so an output is released
only once its producing input prefix is replicated. On recovery, a replacement
...
\Procedure{OnDeliver}{$(\mathit{ts}, \mathit{op})$}\Comment{fabric delivers in \texttt{ts} order}
  ...
  \If{$\mathit{out}.\mathit{seq} > \mathit{hwm}[\mathit{out}.\mathit{client}]$}\Comment{exactly-once suppression}
     \State \textsc{ReplicateThenRelease}($\mathit{out}$)\Comment{durable write $=$ 2PC phase-1}
